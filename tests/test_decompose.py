...
def test_reconstruction_linear(noisy_signal):
    result = IMF().decompose(noisy_signal)
    assert np.max(np.abs(result.reconstruction - noisy_signal)) < 1e-10


def test_reconstruction_robust(noisy_signal):
    result = IMF(SmoothAbs(H=0.4)).decompose(noisy_signal)
    assert np.max(np.abs(result.reconstruction - noisy_signal)) < 1e-10


def test_shapes_and_stage_info(noisy_signal):
    result = IMF(SmoothAbs(H=0.4)).decompose(noisy_signal, window_sizes=[151, 75, 31])
...
def test_linear_stage_info(noisy_signal):
    result = IMF().decompose(noisy_signal, window_sizes=[63, 31])
    for info in result.stages:
        assert info.iterations == 1
        assert np.isnan(info.final_max_delta)


def test_linearity_of_quadratic_decomposition():
    rng = np.random.default_rng(7)
    x = rng.normal(size=256)
    y = rng.normal(size=256)
    left = IMF().decompose(y).imfs - IMF().decompose(x).imfs
    right = IMF().decompose(y - x).imfs
...
def test_gd_quadratic_matches_closed_form(noisy_signal):
    ...
    closed = IMF().decompose(noisy_signal, window_sizes=[63, 31])
    via_gd = IMF(QuadraticViaGD(), tol=1e-12).decompose(noisy_signal, window_sizes=[63, 31])
...
def test_stage_one_noise_sd():
    ...
    for _ in range(50):
        ...
        smoothed.append(IMF().decompose(noise, window_sizes=[501]).imfs[0])
...
def test_invalid_inputs_raise():
    ...
    with pytest.raises(ValueError):
        IMF().decompose(y, window_sizes=[10])
    with pytest.raises(ValueError):
        IMF().decompose(np.zeros((4, 4)))
    with pytest.raises(ValueError):
        IMF().decompose(np.array([]))
...
def test_boundary_reaches_the_padding(boundary):
    ...
    component = IMF(boundary=boundary).decompose(y, window_sizes=[31]).imfs[0]
...
def test_boundaries_differ_at_the_edges():
    ...
    edge_values = {
        boundary: IMF(boundary=boundary).decompose(y, window_sizes=[31]).imfs[0][:5]
...
def test_max_iter_is_respected(noisy_signal):
    result = IMF(SmoothAbs(H=0.4), max_iter=1).decompose(noisy_signal, window_sizes=[63, 31])
    for info in result.stages:
        assert info.iterations == 1


def test_custom_kernel_flows_through_decomposition(noisy_signal):
    ...
    result = IMF(kernel=Uniform()).decompose(noisy_signal, window_sizes=[7])
    ...
    default = IMF().decompose(noisy_signal, window_sizes=[7]).imfs[0]
